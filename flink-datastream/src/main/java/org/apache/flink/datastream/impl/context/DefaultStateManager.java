    ...
    public <T> Optional<ListState<T>> getState(ListStateDeclaration<T> stateDeclaration)
        ...
        if (stateDeclaration.getRedistributionMode()
            ...
            if (stateDeclaration.getRedistributionStrategy()
                ...
                return Optional.of(operatorStateStore.getUnionListState(listStateDescriptor));
            } else {
                return Optional.of(operatorStateStore.getListState(listStateDescriptor));
            }
        } else {
            return Optional.of(operatorContext.getListState(listStateDescriptor));
        }
    }
    ...
    public <K, V> Optional<MapState<K, V>> getState(MapStateDeclaration<K, V> stateDeclaration)
        ...
        MapStateDescriptor<K, V> mapStateDescriptor =
                new MapStateDescriptor<>(
                        stateDeclaration.getName(),
                        stateDeclaration.getKeyTypeInformation().getTypeClass(),
                        stateDeclaration.getValueTypeInformation().getTypeClass());
        return Optional.of(operatorContext.getMapState(mapStateDescriptor));
    }
    ...
    public <T> Optional<ReducingState<T>> getState(ReducingStateDeclaration<T> stateDeclaration)
        ...
        ReducingStateDescriptor<T> reducingStateDescriptor =
                new ReducingStateDescriptor<>(
                        stateDeclaration.getName(),
                        stateDeclaration.reduceFunction(),
                        stateDeclaration.getTypeInformation().getTypeClass());
        return Optional.of(operatorContext.getReducingState(reducingStateDescriptor));
    }
    ...
    public <IN, ACC, OUT> Optional<AggregatingState<IN, OUT>> getState(
        ...
        AggregatingStateDescriptor<IN, ACC, OUT> aggregatingStateDescriptor =
                new AggregatingStateDescriptor<>(
                        ...
                        stateDeclaration.getTypeInformation().getTypeClass());
        return Optional.of(operatorContext.getAggregatingState(aggregatingStateDescriptor));
    }
    ...
    public <K, V> Optional<BroadcastState<K, V>> getState(
        ...
        MapStateDescriptor<K, V> mapStateDescriptor =
                new MapStateDescriptor<>(
                        ...
                        stateDeclaration.getKeyTypeInformation().getTypeClass(),
                        stateDeclaration.getValueTypeInformation().getTypeClass());
        return Optional.of(operatorStateStore.getBroadcastState(mapStateDescriptor));
    }
