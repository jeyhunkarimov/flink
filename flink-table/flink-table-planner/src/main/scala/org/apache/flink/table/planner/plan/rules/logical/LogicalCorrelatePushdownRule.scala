...
class LogicalCorrelatePushdownRule(operand: RelOptRuleOperand, description: String)
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    def getInputRefs(logicalProject: LogicalProject): Seq[RexInputRef] =
      logicalProject.getProjects.asScala
        .filter(expr => expr.isInstanceOf[RexInputRef])
        .asInstanceOf[Seq[RexInputRef]]

    val requiredFields = deriveRequiredFields(funcScan.getCall, projLeft).getOrElse(return
    )
    val targetExprIndexes = requiredFields.map(
      field =>
        projLeft.getRowType.getFieldList.asScala.indexWhere(
          pfield => pfield.getType.eq(field.getType)))
    val targetExprFlatten = getInputRefs(projLeft).zipWithIndex.map(
      exprWithIdx => new RexInputRef(exprWithIdx._2, exprWithIdx._1.getType))
    ...
    val newProjRight: LogicalProject =
      projRight.copy(projRight.getTraitSet, newScan, projRight.getProjects, projRight.getRowType)
    val newCor = cor.copy(
      cor.getTraitSet,
      projLeft,
      newProjRight,
      cor.getCorrelationId,
      cor.getRequiredColumns,
      cor.getJoinType)

    call.transformTo(newCor)
  }

  private def deriveRequiredFields(
      rexNode: RexNode,
      projLeft: LogicalProject): Option[Seq[RelDataTypeField]] = {
    rexNode match {
      case rx: RexCall =>
...
object LogicalCorrelatePushdownRule {
  val INSTANCE = new LogicalCorrelatePushdownRule(
    operand(
